...
#if defined(NDEBUG)
#error "BTCONE cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in ubitcoinone) are considered zero fee (for relaying and mining)
...
int64_t GetBlockValue(int nHeight)
{
    ...
    } else if (nHeight <= 1051200 && nHeight >= 3) {
        nSubsidy = 6 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    if (nHeight > Params().LAST_POW_BLOCK() && nHeight <= 1051200) {
        ret = 4 * COIN;
...
{
    RenameThread("bitcoinone-scriptch");
    scriptcheckqueue.Thread();
}

bool RecalculateBTCONESupply(int nHeightStart)
{
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // BTCONE: We use certain sporks during IBD, so check to see if they are
