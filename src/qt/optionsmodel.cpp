...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::BTCONE);
    ...
    if (!settings.contains("nAnonymizeBTCONEAmount"))
        settings.setValue("nAnonymizeBTCONEAmount", 1000);

    nAnonymizeBTCONEAmount = settings.value("nAnonymizeBTCONEAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeBTCONEAmount"))
        SoftSetArg("-anonymizebitcoinoneamount", settings.value("nAnonymizeBTCONEAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in bitcoinone.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeBTCONEAmount:
            return QVariant(nAnonymizeBTCONEAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizeBTCONEAmount:
            nAnonymizeBTCONEAmount = value.toInt();
            settings.setValue("nAnonymizeBTCONEAmount", nAnonymizeBTCONEAmount);
            emit anonymizeBTCONEAmountChanged(nAnonymizeBTCONEAmount);
            break;
        case CoinControlFeatures:
